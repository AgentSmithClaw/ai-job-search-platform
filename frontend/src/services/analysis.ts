...
export async function generateQuestions(payload: {
  ...
  const { data } = await api.post<{ questions: string[] }>('/generate-questions', payload);
  return data;
}
...
export async function getOrders(): Promise<PaymentOrder[]> {
  const { data } = await api.get<PaymentOrder[]>('/payment/orders');
  return data;
}

export async function refundOrder(orderId: string): Promise<{ status: string; order_id: string }> {
  const { data } = await api.post<{ status: string; order_id: string }>(`/payment/refund/${orderId}`);
  return data;
}
